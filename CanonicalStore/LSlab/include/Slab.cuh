...
struct UniqueHostDevicePtr {
    ...
    inline void moveToGPUAsync(cudaStream_t stream = cudaStreamDefault) {
        gpuErrchk(cudaMemcpyAsync(k, h, s, cudaMemcpyHostToDevice, stream));
    }

    inline void moveToCPUAsync(cudaStream_t stream = cudaStreamDefault) {
...
public:
    ...
    inline SlabUnified<K, V> &operator=(SlabUnified<K, V> &&other) noexcept {
        gpuErrchk(cudaSetDevice(other._gpu));
     ...
     * @param hashes
     */
    inline void batch(BatchBuffer<K, V, UseHost> *buffer) {
     ...
     * @param hashes
     */
    inline void batch(BatchBuffer<K, V, UseHost> *buffer, unsigned blocks,
     ...
     * @param time
     */
    inline void batch(BatchBuffer<K, V, UseHost> *buffer, float &time) {
     ...
     * @param time
     */
    inline void batch(BatchBuffer<K, V, UseHost> *buffer, float &time, unsigned blocks,
                      unsigned threads_per_block, cudaStream_t stream) {
