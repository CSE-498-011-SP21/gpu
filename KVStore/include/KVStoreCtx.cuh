...
public:
    ...
    std::unique_ptr<KVStoreInternalClient<K, M, Slab_t>> getClient() {
        return std::make_unique<KVStoreInternalClient<K, M, Slab_t>>(k.getSlab(), k.getCache(), k.getModel(),
                                                                             num_threads_);
    }

    std::unique_ptr<NoCacheKVStoreInternalClient<K, M, Slab_t>> getNoCacheClient() {
        return std::make_unique<NoCacheKVStoreInternalClient<K, M, Slab_t>>(k.getSlab(), k.getCache(),
                                                                                    k.getModel(),
                                                                                    num_threads_);
    }

    std::unique_ptr<JustCacheKVStoreInternalClient<K, M, Slab_t>> getJustCacheClient() {
        return std::make_unique<JustCacheKVStoreInternalClient<K, M, Slab_t>>(k.getSlab(), k.getCache(),
